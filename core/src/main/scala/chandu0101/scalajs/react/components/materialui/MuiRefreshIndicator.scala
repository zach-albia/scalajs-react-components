...
case class MuiRefreshIndicator(
  ...
  def apply() = {
    ...
    val f = JsComponent[js.Object, Children.None, Null](Mui.RefreshIndicator)
    f(props)
  }
}
...
@js.native
trait MuiRefreshIndicatorM extends js.Object {
  def getArcDeg(): Double = js.native

  def getCircleAttr(): Double = js.native

  def getCircleStyle(): CssProperties = js.native

  def getFactor(): Double = js.native

  def getPaddingSize(): Double = js.native

  def getPaperSize(): Double = js.native

  def getPolygonStyle(): CssProperties = js.native

  def getTheme(): MuiTheme = js.native

  def renderChildren(): Unit = js.native

  def rotateWrapper(wrapper: js.Any): Unit = js.native

  def scalePath(path: js.Any, step: Int): Unit = js.native
}
